...
class ClientProtocol(asyncio.Protocol):
		...
		if mtype == 'CIPHER':
			...
			if success:
				...
				if mtype == 'OK':
					...
					self.process_nonce(decrypt_message)
					return
				elif mtype == 'SIGNATURE':
					self.process_siganture(decrypt_message)
					return
				elif mtype == 'CA':
					self.process_ca(decrypt_message)
					return
			else:
	...
	def process_public_key(self, message) -> None:
		...
		if self.state == STATE_CONNECT:
			...
			if x == '1':
				...
				self._send(self.cipher.encrypt_message({'type': 'CHALLENGE', 'data': self.rsa.export_key()}))
				logger.info("Send challenge!")
			else:
				self.certs = MyCerts(cc=True)
				self._send(self.cipher.encrypt_message({'type': 'CA', 'data': self.certs.get_cert()}))
				logger.info("Send ca!")
		elif self.state == STATE_DATA:
			self.send_file(self.file_name)

	def process_nonce(self, message) -> None:
		if self.type == '1':
			self.rsa.set_nonce(message['data'])
			while True:
			...
			while True:
				password = getpass()
				if password != '': break
			self._send(self.cipher.encrypt_message({'type': 'SIGNATURE', 'username': username, 'signature': self.rsa.signature(password.encode())}))
		else:
			self._send(self.cipher.encrypt_message({'type': 'SIGNATURE', 'signature': self.certs.sign(message['data'])}))
		logger.info("Send SIGNATURE!")

	def process_ca(self, message) -> None:
		self.certs.set_cert(message['data'])
	...
	def process_siganture(self, message: dict) -> None:
		valid, text = self.certs.verification(message['signature'])
		if not valid:
			logger.info(text)
			self.state = STATE_CLOSE
			self.transport.close()
